 ...
 *
 *  $Id: usbdrvlinux.c,v 1.7 2000/05/14 04:12:53 kclayton Exp $	 
 *
 *  History:
			...
			pid = bswap_16(pid);
                        ...
                        if ((vid == vendorid || vendorid == 0xffff) &&
                            (pid == productid || productid == 0xffff)) {
                                return usb_open_bynumber(strtoul(de->d_name, NULL, 0), strtoul(de2->d_name, NULL, 0), vendorid, productid);
                        }
                        close(fd);
                }
                closedir(d2);
...
lprintf(10, "usb_control: rqt 0x%02x rq 0x%02x val 0x%04x idx 0x%04x len %u ret %d\n",
  requesttype, request, value, index, length, i);
	if (i < 0) {
		lprintf(2, "ioctl: USBDEVFS_CONTROL (rqt=0x%x rq=0x%x val=%u idx=%u len=%u) error %s\n",
...
lprintf(10, "usb_bulk: ep 0x%02x len %u ret %d\n", ep, dlen, i);
	if (i < 0) {
		lprintf(2, "ioctl: USBDEVFS_BULK (ep=0x%x len=%u) error %s\n", ep, dlen, strerror(errno));
		return -1;
	}
	return i;
}
...
int usb_resetep(struct usbdevice *dev, unsigned int ep)
{
	...
	if (i < 0) {
		lprintf(2, "ioctl: USBDEVFS_RESETEP (ep=0x%x) error %s\n", ep, strerror(errno));
		return -1;
	}
	return 0;
}
...
int usb_setconfiguration(struct usbdevice *dev, unsigned int configuration)
{
	...
	if (i < 0) {
		lprintf(2, "ioctl: USBDEVFS_SETCONFIGURATION (cfg=%d) error %s\n",
...
int usb_setinterface(struct usbdevice *dev, unsigned int intf, unsigned int altsetting)
{
	...
	if (i < 0) {
		lprintf(2, "ioctl: USBDEVFS_SETINTERFACE (intf=%d,altsetting=%d) error %s\n",
			intf, altsetting, strerror(errno));
		return -1;
	}
	return 0;
}
...
struct usbdevfs_urb *usb_reapurb(struct usbdevice *dev, unsigned int nonblock)
{
	int ret;
